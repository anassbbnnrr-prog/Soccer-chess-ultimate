...
function updatePieceButtons(color) {
    ...
    if (hint) {
        hint.textContent = gameMode === "PAWN_MODE"
            ...
            : `دور ${color === "white" ? "الأبيض" : "الأسود"}: اختر القطع وضعها في نصف ملعبك.`;
    }
}
...
function resize() {
    if (!canvas) return;
    cellSize = Math.floor(Math.min((window.innerWidth * 0.9) / COLS, (window.innerHeight * 0.75) / ROWS));
...
function executeMove(piece, move) {
    ...
    recordHumanMove(move.type); // تسجيل حركة اللاعب الإنساني للتعلم
...
// --- فلترة القطع المرشحة بذكاء: فقط القطع ذات الصلة بالكرة ---
...
function playAITurnForColor(color) {
    let matchStyle = color === "black" ? aiMatchStyle : "POSSESSION"; // الأبيض دائماً استحواذ في AI vs AI
    let style = pickAIStyle(color, matchStyle);

    if (color === "black") showAIStyleBadge(style, color);
    ...
    aiPieces.forEach(p => {
        ...
        candidates.forEach(m => {
            // في وضع AI vs AI نضيف عشوائية أكبر لمنع التكرار
    ...
    if (bestMove && bestPiece) { selectedPiece = bestPiece; executeMove(bestPiece, bestMove); }
    else endCurrentTurn();
}
